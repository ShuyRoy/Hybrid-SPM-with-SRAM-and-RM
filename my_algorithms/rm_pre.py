...
access_sequence1=[]
count = 0
with open(file_path + "//bitcount_new14.txt",encoding='gb18030',errors='ignore') as file_object:
...
with open(file_path + "//basicmath_new14.txt",encoding='gb18030',errors='ignore') as file_object:
    # print("scccccccccc")
    for line in file_object.readlines():
        line = line.replace(' ','').strip()   #去除全部空格
        line = line.replace("'",'')
...
# print(count)
f = open(file_path + "//bitcount_new15.txt",'a')
...
f.close()
print("Success!")
f = open(file_path + "//basicmath_new15.txt",'a')
