...
if __name__ == "__main__":
    ...
    Erw = 1140 # RM写的energy  单位是pJ
    Ers = 328.62 #RM移动操作的energy  单位是pJ
    Es = 226 # SRAM的energy  单位是pJ
    ...
    print("FCFS的cost：",cost0,"FCFS的shift次数：",Ns0)
    ...
    shift_num = compute_shift(data_SRAM, p, access_sequence)
    cost1,energy1=compute_cost(shift_num, Nr, Nw, data_SRAM, Trr, Twr, Tsr, Ts, D)  # 使用全部SRAM时的cost
    area = Ar*(len(D)-len(data_SRAM))+As*len(data_SRAM)
    print("不使用SRAM时的延迟",cost0,"不使用SRAM时的移动次数",Ns0)
    print("SRAM上的数据",data_SRAM)
    print("RM上的放置",p)
    print("SRAM全部使用时的移动次数",shift_num,"SRAM全部使用时的延迟",cost1,"SRAM全部使用时的能耗",energy1,"SRAM全部使用时的面积",area)
    #os.system("python LWSR-2.py {} {} {} {} {} {} {}".format(cost0,cost1,Ns0,shift_num,data_SRAM,p0,access_sequence))
    ...
    #     print("cost减少的百分比：", cost_reduce, "shift减少的百分比：", shift_reduce, "cost：", cost2, "shift：", Ns2, "SRAM上数据量：",
    #           len(data_SRAM2),"ennergy",energy2,"total area", area)
